...
from stacked_capsule_autoencoders.capsules.data.shapenet_data import *
...
def _create_shapenet(subset, batch_size, **kwargs):
    sn_dataset_builder = ShapeNetImageDataset()
    sn_dataset_builder.download_and_prepare()
    return sn_dataset_builder.as_dataset(split = "train", **kwargs).repeat().batch(batch_size)
